...
\subsection{Get fastq file information}
...
<<eval=false>>=
files = list.files(system.file('extdata', package = "SRAdb"), pattern='.bam$', full.names=TRUE)
load2IGV(files, locus='chr1:1-1000000', Merge=TRUE)
load2IGV(files, locus='HES4', Merge=FALSE)
@

If there is no IGV process already running, the following statement will launch a new IGV and load BAM files to it: 
<<eval=false>>=
files = list.files(system.file('extdata', package = "SRAdb"), pattern='.bam$', full.names=TRUE)
