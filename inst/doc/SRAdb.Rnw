...
\begin{figure}
  ...
  \caption{A graphical representation (sometimes called an \textit{Entity-Relationship Diagram}) of the relationships between the main tables in the SRAdb package.}
  \label{figure:ERD}
\end{figure}
...
The \Rfunction{dbListTables} function lists all the tables in the SQLite database handled by the connection object \Robject{sra\_con} created in the previous section. Please see the relationship of the SRA main data types in the Figure 1.
<<>>=
